...
import eslintOption from "../../../lint/lint-eslint-option.js";
...
Main.prototype = {
    ...
    updateBuildButton: function() {
        ...
        const buildCmd = `sf build ${this.getSelectedNames()}${minify}${bundle}`;
        $(".cli-build").attr("tooltip", buildCmd);
        const buildAllCmd = `sf build${minify}${bundle}`;
    ...
    updateTestButton: function() {
        ...
        const testCmd = `sf test ${this.getSelectedNames()}${spec}${debug}${open}`;
        $(".cli-test").attr("tooltip", testCmd);

        const testAllCmd = `sf test${debug}${open}`;
    ...
    updateLintButton: function() {
        ...
        let eslint = this.lintEslintComBox.getValue();
        if (eslint) {
            eslint = ` -e ${eslint}`;
        } else {
            eslint = "";
        ...
        const lintCmd = `sf lint ${this.getSelectedNames()}${eslint}${stylelint}${naming}`;
        $(".cli-lint").attr("tooltip", lintCmd);
        const lintAllCmd = `sf lint${eslint}${stylelint}${naming}`;
    ...
    updatePreCommitButton: function() {
        ...
        const preCommitCmd = `sf precommit ${this.getSelectedNames()}${pass}${minify}`;
        $(".cli-precommit").attr("tooltip", preCommitCmd);

        const preCommitAllCmd = `sf precommit${pass}${minify}`;
    ...
    updatePreviewButton: function() {
        ...
        const previewCmd = `sf preview ${this.getSelectedNames()}${env}`;
        $(".cli-preview").attr("tooltip", previewCmd);
    },
    ...
    updateListButton: function() {
        ...
        const listCmd = `sf list ${this.getSelectedNames()}${sort}`;
        $(".cli-list").attr("tooltip", listCmd);
        const listAllCmd = `sf list${sort}`;
    ...
    showContextMenu: function(d) {
        ...
        const contextMenuLayout = function(contextMenu, e) {
            let top = e.pageY;
        ...
        const contextMenuCloseHandler = function(e) {
            if (!self.contextMenu) {
                return;
            }
    ...
    lintEslintComBoxHandler: function() {
        ...
        if (!this.lintEslintComBox) {
            ...
            this.lintEslintComBox.setList([
                "",
                ... Object.keys(eslintOption)
            ]);
        }

        const eslint = this.getCookie("lint_eslint");
        if (eslint) {
    ...
    envComBoxHandler: function() {
        ...
        const env = this.getCookie("preview_env");
        if (env) {
    ...
    logCleanHandler: function() {
        ...
        const max = this.info.guiHistory || 2000;
