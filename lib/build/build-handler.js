...
const createWebpackConf = function(item, option) {
    ...
    option = Object.assign(option, {
        ...
        externals: externals,
        alias: Util.getConfig('moduleAlias'),
        esModule: Util.getConfig('esModule'),
...
const showConsoleReport = (job, statsReport, filename) => {
    ...
    //check vendors
    const vendors = Util.getConfig('vendors');
...
const buildPreviewFilesHandler = async (item) => {
    ...
    cleanJsAndCssFiles(path.resolve(item.previewOutputPath, filename));
    ...
    const webpackConf = createWebpackConf(item, {
        filename: filename,
        outputPath: item.previewOutputPath,
        mode: getMode(item)
    });
    ...
    const reportCode = await saveStatsReport(item, webpackConf.report, filename, 'reportPreview', true);
...
const buildHandler = (item) => {
    ...
    const tasks = [(it) => {
        return Util.runHook('beforeBuild', it, it.name);
    }, () => {
        ...
        return Util.runHook('afterBuild', it, it.name);
    }];
