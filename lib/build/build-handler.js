...
const createWebpackConf = function(item, option) {
    ...
    option = Object.assign(option, {
        ...
        esModule: Util.getSetting('esModule'),
...
const showConsoleReport = (job, statsReport, filename) => {
    ...
    //check vendors
    const vendors = Util.getSetting('vendors');
...
const buildHandler = (item) => {
    ...
    const tasks = [(it) => {
        return Util.runHook('beforeBuild', it.name, it);
    }, () => {
        ...
        return Util.runHook('afterBuild', it.name, it);
    }];
