...
const cleanFilesAfterBuild = (item, filename, currentFiles) => {
    const jsFile = Util.relativePath(path.resolve(item.buildPath, filename));
...
const buildModuleFiles = async (item, entry) => {
    ...
    cleanFilesAfterBuild(item, filenameModule, item.moduleFiles);
...
const buildFilesHandler = async (item) => {
    ...
    cleanFilesAfterBuild(item, filenameMain, item.buildFiles);
