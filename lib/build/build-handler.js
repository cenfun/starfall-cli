...
const buildDevFilesHandler = async (item) => {
    ...
    //for production is page main file
    const filename = `${item.buildName}.main.js`;
    
    // check if has dev entry src/index.js
    const entry = `${item.devPath}/src/${item.entryFile}`;
    ...
    Util.logEnd(`finish: ${item.devFiles.join(', ')}`);

    cleanBuildFiles(item.devBuildPath, filename, item.devFiles);
...
const buildHandler = (item) => {
    ...
    const tasks = [() => {
        ...
        return buildDevFilesHandler(item);
    }, () => {
