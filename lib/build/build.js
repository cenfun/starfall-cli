...
const getJobList = async (list) => {
    ...
    const buildPath = Util.getConfig('buildPath');
    const previewPath = Util.getConfig('previewPath');
    const srcEntry = Util.getConfig('srcEntry');

    const cssExtract = Util.getConfig('cssExtract');
    const externals = Util.getConfig('externals');
    ...
    for (let i = 0, l = list.length; i < l; i++) {
        ...
        const job = {
            ...
            previewOutputPath: `${componentPath}/${previewPath}/${buildPath}/`,
...
const buildList = async (list) => {
    ...
    const beforeCode = await Util.runHook('beforeBuildAll', jobList);
    if (beforeCode) {
        Util.logRed('ERROR: Failed to run hook beforeBuildAll');
        return beforeCode;
    }
    ...
    return Util.startWorker({
        ...
        reportHandler: async (o) => {
            ...
            const afterCode = await Util.runHook('afterBuildAll', o);
            if (afterCode) {
                o.code = afterCode;
                o.exitError = 'ERROR: Failed to run hook afterBuildAll';
            }
