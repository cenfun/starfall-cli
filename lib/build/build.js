...
const getJobList = async (list) => {
    ...
    const extra = Util.option.extra;

    //default bundle only vendor (deps)
    const bundle = Util.option.bundle;
    //bundle all
    const all = bundle && Util.option.all;
    const query = Util.option.query;
    const css = Util.option.css || Util.getSetting("cssExtract");
    const inject = Util.option.inject || Util.getSetting("injectAlignedPath");
    ...
    for (let i = 0, l = list.length; i < l; i++) {
        ...
        const job = {
            ...
            inject: inject || componentConf.injectAlignedPath,
            command: command
        };
...
const buildList = async (list) => {
    ...
    let workerLength;
    if (jobList.length > 1 && Util.option.bundle && Util.option.all) {
        workerLength = 1;
    }
...
const buildModule = async (componentName) => {
    ...
    //build only one if all in one or bundle
    if (Util.option.all || Util.option.bundle) {
