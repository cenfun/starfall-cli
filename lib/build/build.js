...
import reportHandler from './build-report.js';
...
const getBuildDefine = (env) => {
    ...
    if (version !== env.version) {
        console.log(`new version: ${env.version} => ${EC.cyan(version)}`);
    }

    const define = {
        ... buildDefine,
        'window.VERSION': version,
        'window.COMMIT': env.commit,
        'window.BRANCH': env.branch,
        'window.TIMESTAMP': env.timestamp
    };

    Object.keys(define).forEach((k) => {
        define[k] = JSON.stringify(define[k]);
    });
...
const getJobList = async (list, options = {}) => {
    ...
    for (let i = 0, l = list.length; i < l; i++) {
        ...
        // if build es module files
        const esm = Util.option.esm || componentConf.type === 'module';
