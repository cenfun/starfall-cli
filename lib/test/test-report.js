...
import {
    state, isList, log, logObject, getTempRoot, TF, getCoveragePercent
} from '../core/util.js';

const getExitError = (job, option) => {
    const list = [`ERROR: Unit test failed (${job.name})\n`];
    if (isList(option.failedList)) {
...
const generateTestReport = (job, summary) => {
    ...
    log(job.name, 'unit test summary');
    job.summary = summary;
    logObject(summary);
};
...
const generateCoverageReport = async (job, coverage) => {
    ...
    // Not found v8 coverage data
    if (!coverage) {
        return;
    }

    log(job.name, 'coverage report');

    const coveragePath = `${getTempRoot()}/coverage/${job.name}`;
...
const generateReport = (option) => {
    ...
    rows = rows.map(function(job, i) {
        ...
        // MCR summary report: coverage-summary.json
        const jcr = job.coverageReport;
        if (jcr) {
            // for summary
            item.statements = getCoveragePercent(jcr.statements.covered, jcr.statements.total);
            item.functions = getCoveragePercent(jcr.functions.covered, jcr.functions.total);
            item.branches = getCoveragePercent(jcr.branches.covered, jcr.branches.total);
            item.lines = getCoveragePercent(jcr.lines.covered, jcr.lines.total);
            ...
            item.coverage = getCoveragePercent(itemCovered, itemTotal);
    ...
    const coverage = getCoveragePercent(overall.covered, overall.total);
