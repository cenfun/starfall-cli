(function(window) {
    ...
    const loadSpec = function(src, testAPI, callback) {
        testAPI.sendMessage(`load ${src} ...`);
        const script = document.createElement('script');
        script.src = src;
        script.onload = callback;
        script.onerror = function(e) {
            testAPI.sendMessage(EC.red(`ERROR: failed to load: ${src}`));
            callback();
        };
    ...
    const mochaReporter = function(config, testAPI) {
        ...
        return function(runner) {
            ...
            runner.on(constants.EVENT_TEST_FAIL, function(test, err) {
                ...
                testAPI.sendMessage(EC.red(errorMsg));
                failedList.push({
                    title: test.title,
                    errorMsg: errorMsg
                });
    ...
    const startMocha = function(config, testAPI) {
        ...
        loadSpec(config.buildName, testAPI, function() {
            mocha.run();
        });
        ...
        sendFinish: function(time_start, summary, failedList, config) {
            ...
            if (config.debug) {
                if (failedList.length) {
                    testAPI.sendMessage(EC.red(getExitError(failedList)));
                } else {
                    testAPI.sendMessage(EC.green('Congratulations! All tests passed.'));
                }
