...
const generateCoverageReport = async function(data, testUrl, coverageDir, pkg) {
    if (!data.length) {
    ...
    const report = MCR({
        name: `${pkg.name || path.basename(state.root)} unit test coverage`,
...
const testModule = async () => {
    ...
    const configFile = path.resolve(state.root, 'vite.config.js');
    const coverageDir = path.resolve(state.root, '.temp/coverage');
    const pkg = JSON.parse(fs.readFileSync(path.resolve(state.root, 'package.json'), 'utf8'));
    const debug = state.option.debug;
    const debugSpecFilter = typeof debug === 'string' ? debug : '';
    ...
    if (coverageData.length && testUrl) {
        try {
            ...
            await generateCoverageReport(coverageData, testUrl, coverageDir, pkg);
            const coverageReportPath = path.relative(state.root, path.resolve(coverageDir, 'index.html')).replace(/\\/g, '/');
