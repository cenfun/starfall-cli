...
const launchBrowser = async (job) => {
    ...
    Util.logMsg(job.name, `[browser] ${EC.green("launched")} ${v}${Util.getCost(time_start, 3000)}`);
    return browser;
};
...
const finishTest = async (job, code) => {
    ...
    Util.finishBrowserUserDataDir(job.tracesDir);
