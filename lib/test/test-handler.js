...
const launchBrowser = async (job) => {
    ...
    const browserType = Util.getBrowserType(job.browser);
    const browser = await playwright[browserType].launch(browserOption);
    
    const v = await browser.version();
    Util.logMsg(job.name, `[browser] ${EC.green("launched")} ${browserType} ${v}${Util.getCost(time_start, 3000)}`);
    return browser;
};
...
const finishTest = async (job, code) => {
    ...
    Util.finishBrowserDataCacheDir(job.cacheDir);
