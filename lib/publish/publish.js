...
const buildModule = require('../build/build.js');
// ==============================================================================================
...
const publishItem = (item) => {
    ...
    // go to folder first
    const componentPath = Util.option.root ? Util.root : Util.getComponentPath(item);
    const tasks = [() => {
...
const checkVersion = async (name) => {
    ...
    if (Util.option.override) {
        Util.logYellow('allow override version');
        return 0;
    }
...
const publishProject = async (list, components, newVersion) => {
    ...
    const tasks = [
        ...
        () => {
            // required build with new version
            if (newVersion) {
                return buildModule.buildList(list);
            }
            return 0;
        },
        () => {
