...
const publishItem = (item) => {
    ...
    if (Util.option.debug) {
        ...
        let cmd = `npm publish --registry ${Util.option.registry}`;
        if (Util.option.tag) {
...
const checkVersion = async (name) => {
    ...
    const registry = Util.getSetting('registry');
    const url = `${registry}${fullName}`;
...
const publishModule = function(newVersion) {
    ...
    const registry = Util.getSetting('registry');
    if (!registry) {
        Util.logRed('publish require registry in conf.cli.js');
        process.exit(1);
        return;
    }

    Util.option.registry = registry;
    Util.logGreen(`publish registry:${registry}`);
