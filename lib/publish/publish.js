...
const publishProject = async (list, newVersion) => {
    ...
    const tasks = [() => {
        ...
        console.log("start precommit list ...");
        const preCommit = Util.getSetting("preCommit");
        if (preCommit === false || Util.option.force) {
            Util.logYellow("Disabled pre-commit");
            return 0;
        }
        return precommitModule.precommitList(list, preCommit);
    }, () => {
        if (newVersion) {
...
const publishModule = function(newVersion) {
    ...
    //build minify file default
    Util.option.minify = true;
    if (Util.option.bundle) {
        Util.option.bundle = "publish";
    }
