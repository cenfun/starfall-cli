...
const namingHandler = (item) => {
    ...
    console.log(`${EC.cyan(`[${item.name}] `) + EC.magenta('naming')} ${Util.relativePath(componentPath)}`);
...
const stylelintHandler = (item) => {
    ...
    console.log(`${EC.cyan(`[${item.name}] `) + EC.magenta('stylelint')} ${shortCmd(cmd)}`);
...
const eslintHandler = (item) => {
    ...
    console.log(`${EC.cyan(`[${item.name}] `) + EC.magenta('eslint')} ${shortCmd(cmd)}`);
    const sh = shelljs.exec(cmd);
    return sh.code;
};
