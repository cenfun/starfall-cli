...
import reportHandler from './lint-report.js';
import lintHandler from './lint-handler.js';

const getEslintConfPath = function() {
    const confFileName = 'eslint.config.js';
    // from project
    let confPath = `${Util.root}/${confFileName}`;
    if (!fs.existsSync(confPath)) {
        // default config
        Util.logYellow(`Not found ${confFileName}`);
...
const getStylelintConfPath = function() {
    const confFileName = 'stylelint.config.js';
    // from project
    let confPath = `${Util.root}/${confFileName}`;
    if (!fs.existsSync(confPath)) {
        // default config
