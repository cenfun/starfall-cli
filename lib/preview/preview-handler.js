...
const previewStart = async (componentName, env) => {
    ...
    //==================================================================
    const previewApiCallback = Util.getConfig('previewApiCallback');
    if (typeof previewApiCallback === 'function') {
        const previewApi = previewApiCallback();
        if (previewApi) {
...
const previewHandler = (componentName, restart) => {
    ...
    previewStart(componentName, env);

};

previewHandler.sendMessage = function(data) {
    if (sockets) {
