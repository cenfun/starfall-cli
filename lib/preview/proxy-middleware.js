...
const proxyMiddleware = function(app, env) {
    ...
    app.use(async (ctx, next) => {
        ...
        //check userid
        if (!ctx.req.headers["x-api-userid"]) {
            const userid = ctx.cookies.get("userid");
            if (userid) {
                ctx.req.headers["x-api-userid"] = userid;
                //console.log('set request header x-api-userid: ' + userid);
            }
        }
    ...
    for (const context in confProxy) {
        const item = confProxy[context];
        const option = item[env] || item.default || item.DEV || item;
        if (option) {
