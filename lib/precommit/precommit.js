...
const precommitList = (list, preCommit = {}) => {

    console.log('precommit = lint + build + test');
...
const precommitStart = async (list) => {
    ...
    const preCommit = Util.getSetting('preCommit');
    if (preCommit === false) {
        Util.logYellow('Disabled pre-commit');
        return;
    }
    ...
    const exitCode = await precommitList(list, preCommit);
    if (exitCode === 0) {
...
precommitModule.precommitList = precommitList;
