...
const packComponent = async (componentName) => {
    ...
    // always build first
    const item = await build.buildComponent(componentName, true);
    if (!item) {
        return 1;
    }

    const beforeCode = await Util.runHook('pack.before', item);
    if (beforeCode) {
        Util.logRed('ERROR: Failed to run hook pack.before');
        return beforeCode;
    }
    ...
    // zip folder
    await packFiles(item);


    const afterCode = await Util.runHook('pack.after', item);
    if (afterCode) {
        Util.logRed('ERROR: Failed to run hook pack.after');
        return afterCode;
    }
