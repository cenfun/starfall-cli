...
const copyStaticFiles = (item) => {
    ...
    staticFiles.forEach(function(filePath) {
        ...
        //do not copy from node_modules if bundle all
        if (item.all && filePath.indexOf("node_modules") !== -1) {
            return;
        }
...
const copyDependencies = (item) => {
    ...
    if (item.noNeedDependenciesFiles) {
        return;
    }
...
const copyBuildFiles = (item, fromPath, buildPath) => {
    ...
    if (item.buildCssFile) {
        item.buildCssFile = updateBuildFilePath(item, item.buildCssFile, fromPath);
    }
    if (item.buildFile) {
        item.buildFile = updateBuildFilePath(item, item.buildFile, fromPath);
    }
    if (item.previewFile) {
        item.previewFile = updateBuildFilePath(item, item.previewFile, fromPath);
    }
};
