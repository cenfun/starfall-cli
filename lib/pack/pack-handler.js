...
import injectHandler from '../inject/inject.js';
...
const copyServerFiles = (item) => {
    ...
    const proxyFile = `${Util.root}/scripts/conf.proxy.js`;
    if (fs.existsSync(proxyFile)) {
        const proxyPath = path.resolve(packPath, 'scripts');
        if (!fs.existsSync(proxyPath)) {
            fs.mkdirSync(proxyPath);
        }
        shelljs.cp('-f', proxyFile, proxyPath);
