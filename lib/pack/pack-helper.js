...
const minifyJs = async (absFilePath) => {
    const code = Util.readFileSync(absFilePath);
    if (!code) {
        return;
    }
    Util.log(`minify js with Terser: ${Util.relativePath(absFilePath)} ...`);
    ...
    Util.log(`minified js: ${path.basename(absFilePath)}`);
};
...
const minifyCss = async (absFilePath) => {
    ...
    Util.log(`minify css with PostCSS/CssNano: ${Util.relativePath(absFilePath)} ...`);
    ...
    Util.log(`minified css: ${path.basename(absFilePath)}`);
};
