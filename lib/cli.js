...
//cli root
Util.cliRoot = path.resolve(__dirname, "../");
...
const runTask = function(cmd, argList) {
    ...
    Util.logStart(`sf ${cmd} ${args.join(" ")}`);
...
program.on("--help", function() {
    ...
    const commands = this.commands.filter(function(cmd) {
        ...
        const name = `${programName} ${EC.cyan(cmd._name)}${args ? ` ${args}` : ""}`;
        const command = {
            name: name,
    ...
    const sfOptions = this.options.map(function(o) {
        return {
            name: ` ${o.flags}`,
            description: o.description
        };
...
program
    ...
    .option("-b, --bundle [publish]", "bundle vendor, publish: empty dependencies")
    .option("-a, --all", "bundle all in one")
    .option("-q, --query <string>", "query string")
    .option("-c, --css", "css extract")
    .option("-i, --inject", "inject aligned path")
    .option("-e, --extra", "extra minify file")
    .option("-o, --open", "open report when finished")
    .option("--cp <num>", "child process number")
...
program
    .command("init")
    .option("-f, --force", "skip the questionnaire altogether")
...
program
    ...
    .option("-b, --bundle [publish]", "bundle vendor, publish: empty dependencies")
    .option("-a, --all", "all in one")
    .option("-h, --hash [len]", "hash filename")
    .option("-q, --query <string>", "query string")
    .option("-c, --css", "css extract")
...
program
    ...
    .option("-p, --pass", "pass without test")
...
program
    ...
    .alias("p")
    .option("-e, --env <env>", "env for preview proxy")
    .option("-p, --port <port>", "port for preview")
    .option("-c, --css", "css extract")
...
program
    ...
    .option("-f, --force", "force publish without precommit")
    .option("-p, --pass", "pass without test")
    .option("-m, --message <message>", "a message to commit")
    .option("-t, --tag <tag>", "tag package")
    .option("-b, --bundle", "bundle vendor, empty dependencies")
    .option("-a, --all", "bundle all in one")
    .option("-q, --query <string>", "query string")
    .option("-c, --css", "css extract")
    .option("-i, --inject", "inject aligned path")
...
program
    .command("*")
    .action(function() {
        Util.logRed(" unknown command, try sf --help");
    });
