...
program
    .command('audit <url>')
    .alias('a')
    .option('-c, --config', 'specified config for Lighthouse')
    .description('audit a page with Lighthouse')
    .action(function() {
        runTask('audit', arguments);
    });
...
program
    ...
    .option('-c, --chrome', 'launch with chrome')
    .option('-w, --watch <glob>', 'watch additional path')
    .option('-m, --monitor', 'monitor for chrome (TBC)')
...
program
    .command('pack [name]')
    .option('-b, --bundle [type]', 'bundle all or vendor')
