...
//const devMem = require('./dev-mem.js');
...
const initLiveReload = (componentName) => {
    ...
    watch(list, function(res) {
        ...
        Util.log(`${EC.magenta(res.name)} ${EC.yellow(type)} for ${EC.magenta(res.folder)} ${res.type}: ${res.path}`);
...
const showIpInfo = async (url, port) => {
