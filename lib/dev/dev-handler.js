...
const getRefreshType = (res, componentName) => {
    ...
    if (res.name === componentName && res.folder === devPath) {

        // dev/src
        if (res.path.indexOf(`${devPath}/src/`) !== -1) {
            return 'rebuild';
        }
...
const devStart = async (componentName, env) => {
    ...
    server.listen(port, function() {
        Util.logStart(`${new Date().toString()} dev server started`);
