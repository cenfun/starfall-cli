...
const generateDiffList = (item, aFiles, bFiles) => {
    ...
    Object.keys(bFiles).forEach(function(k) {
        ...
        Util.log('[diff]', `${EC.green('generated')} ${k} and cost: ${duration}`);
    });
    return list;
};
...
const generateHtmlReport = (item, report, patch) => {
    ...
    const styleContent = Util.readFileSync(stylePath);
    ...
    Util.writeFileSync(reportPath, html);
...
const diffVersion = async (item, repJson) => {
    ...
    Util.writeFileSync(`${item.outputPath}/diff.patch`, patch);
...
const downloadRepJson = async (item) => {
    const url = item.registry + item.name;
    const res = await axios({
        method: 'get',
        url: url,
        timeout: 10 * 1000,
    ...
    Util.writeJSONSync(confPath, info);
