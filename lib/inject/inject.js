...
const getHTMLInjectContent = function(item, type) {
    const injectContent = [];
    item.injectList.forEach(function(item) {
...
const injectComponent = (item) => {
    ...
    //console.log(previewPath, files, pageList);
...
const getInjectList = (item) => {
    ...
    if (item.bundleFiles) {
        ...
        files = files.concat(item.previewFiles);
    }
...
const updatePackage = function(item) {
    ...
    //============================================================
    //main only js
    const main = outputFiles.find(item => {
