...
const checkVersion = function(currentVersion, newVersion) {
    ...
    if (semver.lt(currentVersion, newVersion)) {
        ...
        tips += `  ${currentVersion} => ${EC.bg.green(` ${newVersion} `)}`;
        tips += "\n";
        tips += EC.yellow("▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇\n");
...
const needUpgrade = (currentVersion, currentTimestamp, filePath) => {
    ...
    if (json) {
        if (currentTimestamp - json.timestamp < checkDuration) {
            if (Math.random() > 0.8) {
...
const getLatestVersion = async () => {
    ...
    const url = `${infoRegistry}${Util.name}-cli`;
    const res = await axios.get(url).catch(function() {
