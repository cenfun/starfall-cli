...
const Util = {
    ...
    getComponentRoot: function() {
        const packagesPath = Util.getConfig('packagesPath');

        //check subs
        Util.componentsRoot = Util.formatPath(path.resolve(Util.root, packagesPath));
    ...
    getWebpackExternals: function(modules, componentExternals) {
        ...
        const moduleRootNames = Util.getConfig('moduleRootNames');
    ...
    getHook: function(hookName, componentName) {
        const hooks = Util.getConfig('hooks');
        if (hooks) {
            if (componentName) {
    ...
    runHook: (hookName, option, componentName) => {
    ...
    getComponentDependencies: function(componentName, force) {
        ...
        const moduleOverrides = Util.getConfig('moduleOverrides');

        const entry = Util.getComponentPath(componentName);

        //TODO to moduleExclude
        let exclude = [];
        const injectIgnore = Util.getConfig('injectIgnore');
        if (injectIgnore) {
    ...
    initGitHooks: function() {
        ...
        const addPreCommitHook = Util.getConfig('addPreCommitHook');
        if (!addPreCommitHook) {
            return;
        }
    ...
    isFailFast: (jobName) => {
        ...
        const failFast = Util.getConfig('failFast');
        if (failFast === false) {
            return false;
        }
    ...
    getMultiprocessing: (jobName) => {
        let multiprocessing = Util.getConfig('multiprocessing');
    ...
    forEachFile: function(dir, extList, callback) {
        ...
        list.forEach(function(name) {
            ...
            if (info.isDirectory()) {
                ...
                if (!Util.isList(extList) || extList.includes(path.extname(name)) || extList.includes(name)) {
                    callback(name, dir);
                }
            }
        });
    ...
    launchBrowser: async (option) => {
        ...
        Util.logMsg(option.name, '[browser] launch ...');
        ...
        Util.logMsg(option.name, `[browser] ${EC.green('launched')} ${browserType} ${v}${Util.getCost(time_start, 3000)}`);
        return browser;
    },
