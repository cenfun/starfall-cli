...
const Util = {
    ...
    getWebpackExternals: function(item) {

        const modules = item.dependencies.modules;
        const componentExternals = item.externals;
        ...
        list.forEach(function(dep) {
            const rootName = rootNames[dep];
            if (rootName && rootName !== dep) {
                const newItem = {};
    ...
    getComponentBuildName: function(name, postfix = '') {
    ...
    initGitHooks: function() {
        ...
        const cliConf = path.resolve(Util.root, 'scripts/conf.cli.js');
        if (!fs.existsSync(cliConf)) {
            return;
        }
    ...
    async rm(f, option = {}) {
        let ok = true;
        await rimraf(f, option).catch(function(err) {
            console.error(err);
            ok = false;
        });
        return ok;
    ...
    editFile: function(from, callback, dest) {
        const content = Util.readFileContentSync(from);
        ...
        dest = dest || from;
        Util.writeFileContentSync(dest, editedContent, true);
        return editedContent;
    },

    editJSON: function(from, callback, dest) {
        const json = Util.readJSONSync(from);
        const editedJson = callback.call(this, json);
        // can not compare json object
        dest = dest || from;
        Util.writeJSONSync(dest, editedJson, true);
        return editedJson;
    },

    sortPackageKeys: function(json = {}) {
    ...
    getBrowserDataDir: function(mkdir) {
        const dir = path.resolve(Util.getTempRoot(), 'browser-data-dir');
    ...
    getBrowserDownloadDir: function(mkdir) {
        const dir = path.resolve(Util.getBrowserDataDir(), 'download');
        if (mkdir && !fs.existsSync(dir)) {
            fs.mkdirSync(dir, {
                recursive: true
            });
    ...
    launchBrowser: async (option) => {
        ...
        const browserOption = {
            downloadsPath: Util.getBrowserDownloadDir(),
            args: Util.getBrowserLaunchArgs(),
            ignoreDefaultArgs: Util.getBrowserLaunchIgnoreArgs()
        };

        // devtools for debug
        // If option is true, the headless option will be set false
        if (option.debug) {
    ...
    format: function(target, type = 'eslint') {
        const supportedTypes = {
            eslint: 'npx eslint {placeholder_target} --env browser --fix --color',
            stylelint: 'npx stylelint {placeholder_target} --allow-empty-input --color --fix'
        };

        let cmd = supportedTypes[type];
        if (!type) {
            const ext = path.extname(target);
            const styleExts = ['.css', '.scss'];
            if (ext && styleExts.includes(ext)) {
                cmd = supportedTypes.stylelint;
            } else {
                cmd = supportedTypes.eslint;
            }
        }

        cmd = Util.replace(cmd, {
            placeholder_target: Util.formatPath(target)
        });
