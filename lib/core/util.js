...
const Util = {
    ...
    initStyleLoaderOptions: function(styleLoader, item) {
        ...
        if (options.hasOwnProperty("attrs")) {
            ...
            options.attributes = options.attrs;
        }
    ...
    initHtmlLoaderOptions: function(htmlLoader) {
        const options = htmlLoader.options;
        if (!options) {
            return;
        }

        //update old options
        //https://github.com/webpack-contrib/html-loader#minimize
        const minimize = {};
        const list = [
            "collapseWhitespace",
        ...
        list.forEach(k => {
            if (options.hasOwnProperty(k)) {
                minimize[k] = options[k];
                delete options[k];
            }
        });

        //init new options
        if (Object.keys(minimize).length) {
    ...
    initWebpackConf: function(webpackConf, item) {
        ...
        //html-loader
        rules.map(rule => {
            if (rule.use && rule.use.loader === "html-loader") {
                return rule.use;
            }
        }).filter(item => item).forEach(htmlLoader => {
            Util.initHtmlLoaderOptions(htmlLoader, item);
    ...
    getHash: function(hash) {
        ...
        const hashStr = Util.generateGUID().split("-").join("").substr(0, len);
        return hashStr;
    },
    ...
    isFailFast: (jobName) => {
        ...
        if (failFast && typeof(failFast) === "object") {
            if (failFast[jobName] === false) {
                return false;
            }
        }
        return true;
    ...
    getWorkerOption: function(option) {
        const workerOption = Object.assign({
