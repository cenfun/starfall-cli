...
const generateGitIgnoreItem = (filePath, fileName) => {
...
const generateGitIgnoreItems = (root, deep = false) => {
    console.log(`generating .gitignore (deep: ${deep}) ...`);
    const ls = [];
    ...
    if (deep) {
        ...
        Util.forEachFile(from, ['.gitignore'], function(fileName, filePath) {
            ...
            const item = generateGitIgnoreItem(filePath, '.gitignore');
            if (item) {
                ls.push(item);
                parentIg = item.ig;
            }
        ...
        //root only
        const item = generateGitIgnoreItem(from, '.gitignore');
        if (item) {
            ls.push(item);
        }
    ...
    console.log('generated .gitignore list');
    return ls;
};
