...
import {
    state, readFileSync, logRed, relativePath, isList, DTF
} from '../core/util.js';

const getGitIgnoreContent = (p, n) => {
    const confPath = path.resolve(p, n);
    return readFileSync(confPath);
};
...
const getDirList = (dir) => {
    if (!dir) {
        logRed(`[ignore] Invalid dir: ${dir}`);
        return [];
    }
    if (!fs.existsSync(dir)) {
        logRed(`[ignore] Not found dir: ${dir}`);
        return [];
    }
    ...
    list.forEach(function(subName) {
        ...
        if (info.isDirectory()) {
            if (subName === '.git' || subName === 'node_modules') {
                console.log(`[ignore] ignore ${relativePath(subAbs)}`);
                return;
            }
            subs.push(subName);
            return;
...
const generateGitIgnoreItems = (root, deep = false) => {
    ...
    console.log(`[ignore] generated .gitignore list: ${ls.length} (cost ${DTF(duration)})`);
    return ls;
};
...
const getIgnoreList = (root, breakdownNodeModules) => {
    root = path.resolve(root || state.root);
    const igs = generateGitIgnoreItems(root, true);
    if (!isList(igs)) {
        return [];
    }
    // console.log(igs);
...
let projectIgnore;
const getProjectIgnore = () => {
    if (!projectIgnore) {
        const igs = generateGitIgnoreItems(state.root);
        if (isList(igs)) {
            projectIgnore = igs[0];
        }
    }
...
const getProjectIgnoreRules = function() {
    const item = getProjectIgnore();
    if (!item) {
        return;
    }
